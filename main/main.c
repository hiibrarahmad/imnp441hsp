 ...
 */
 ...
 static int16_t i2s_read_buff[I2S_MIC_BUFFER_SIZE];
 ...
 static void i2s_mic_init()
 {
     ...
     ESP_ERROR_CHECK(i2s_driver_install(I2S_MIC_PORT, &i2s_config, 0, NULL));
     ESP_ERROR_CHECK(i2s_set_pin(I2S_MIC_PORT, &pin_config));
     ESP_ERROR_CHECK(i2s_zero_dma_buffer(I2S_MIC_PORT));
     
     ESP_LOGI(BT_HF_TAG, "I2S microphone initialized");
 }
 
 /* Task to read from microphone and send audio data */
 static void mic_task(void *arg)
 {
     ...
     while (1) {
         ...
         // Here you would typically process the audio data and send it via HFP
         // For now we just print how much data we read
         ESP_LOGD(BT_HF_TAG, "Read %d bytes from microphone", bytes_read);
